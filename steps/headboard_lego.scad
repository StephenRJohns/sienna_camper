...
module s1_parts() {
    wbox([0, 0, 0], [PT, L, HH]);
    cap(str("E  2x side panel ", L, "\" x ", HH, "\" (3/4\" ply)"), 8, -HH * 0.42 - 3, 2.6);
    wbox([0, 0, 0], [W, L, PT], [PT + 10, 0]);
    cap(str("H  2x FULL-DEPTH shelf ", W, "\" x ", L, "\" (3/4\" ply)"), PT + 10 + W * 0.3, -12, 2.6);
}
module s1_assembly() {
    lift = 9;
    panel_c_ctx();
    ...
    iarrow([W/2 - PT/2, y0 + L/2, z_deck + lift - 1], [W/2 - PT/2, y0 + L/2, z_deck + 1]);
    // shelf-to-side-panel screws, both shelves, both ends
    ...
    cap("the 2 full-depth shelves ARE the structure — no full-height divider, no top. 3 full-width tiers, open both faces and above.", 0, -26, 2.0);
}
...
module s3_parts() {
    wbox([0, 0, 0], [W, PT, ndh]);
    cap(str("F  1x nook divider ", W, "\" x ", ndh, "\" (1/2\" ply) + bungee/shock cord + 10 screw-eyes"), W * 0.42, -8, 2.4);
}
module s3_assembly() {
    panel_c_ctx();
    shell_geom(0, true);
