...
| 5 | Fridge empty weight | (Section 8 weight table) | 46.8 lb | |
...
The interior envelope shown uses the measurements from Section 1 (96" x 48.5" x 44" — the 96" length is an UNVERIFIED estimate, see below), but it's still a simplified rectangular box — the real hatch curvature, vent shapes, and wall taper aren't modeled beyond the reserves called out in `params.scad`.
...
The headboard/pantry's shelving, mounted on Panel C's deck at the tailgate end — exploded isometric detail: the shell (just the 2 side panels — no full-height divider, no top) lifted slightly to show the bolt-down base cleats, the 2 full-depth shelves pulled out along their installed axis, and the short nook divider hovering over its top-tier slot (sway braces in the Sway Brace Detail view below):

![Headboard storage detail](renders/headboard-storage-detail.svg)

All 4 sides of the headboard/pantry as flat, dimensioned elevations — mattress-facing (what you see from the bed: the personal nook in the top tier, open netted tiers below), kitchen-facing (the netted food tiers, what the rear view above looks at), and both 14"-deep sides — every piece labeled with its material and cut size:
...
Each floor panel, exploded: frame + legs + center divider + drawers (Panel A's left bay swaps its drawer for WAVE 3 open storage instead; Panel C has no divider or drawers — the fridge/kitchen units live in its void, shown here as reserved footprint):
...
Component 2's one-piece slatted bed platform (58" x 52", 10 slats pocket-screwed between two full-length 1x4 side rails — one flush 3/4" plane, cantilevered 3" past the boxes on each side), exploded. It rests DIRECTLY on Panel A/B's top rails and ENDS at the B/C seam — Panel C's own fixed deck sits at exactly the same surface height, so the two meet flush and the mattress's last ~20" rides that deck. It lifts straight off; leveling happens at the leg feet, down at the floor, and an RV bar bubble level screwed to the driver-side rail's outer edge reads fore-aft pitch while you turn the knobs (its twin on the headboard nook reads roll):
...
**Electric option (controls under Panel A, driver side)**: replace the 12 feet with compact 12V linear actuators (~2" stroke, 750N+ class), wired in per-panel groups of 4 to three 4-switch DPDT momentary pods mounted under Panel A's driver-side rail (reachable from the driver slider), fed from the DELTA 3's 12V circuit like the fridge line. Budget ~$450–600 plus wiring across the two seams. Documented as an **OPTION, not the baseline**: actuators tolerate sustained side loads poorly, need care to keep travel in sync, and the wheel-ramp practice above removes the daily need. If you do go electric, keep 4 manual feet in the corners as mechanical backstops rated for the parked load.
...
- **Headboard/pantry** (14" deep x 46" wide x 22" tall above the deck — as tall as the roof allows, ~2.75" of ceiling clearance to spare against the verified 44" interior height): **no longer its own module** — no frame, no legs, no floor contact of its own. It's a shelving superstructure, **bolted down to Panel C's existing deck** (4x 1/4-20 bolts into T-nuts under the deck) and braced against driving sway by **2 steel L-angle braces running forward/down to Panel C's OWN side rails** (~45°, one per side — triangulates the fore-aft rocking that braking/accel would otherwise work into the tall unit; re-anchored from Panel B, whose rails the flush bed platform now caps — Sway Brace Detail render), occupying the last 14" of Panel C's own 36" length (the end flush to the tailgate, right next to the fridge/kitchen void). **3 full-width tiers — no full-height divider, no top**: two FULL-DEPTH (14") shelves split the 22" height into 3 open tiers. The bottom and middle tiers are full-depth food storage, netted on BOTH faces (with no divider, braking would otherwise slide food onto the bed). The top tier is split by a short nook divider: a 2.75"-deep **personal nook** faces the mattress — the top shelf's surface sits 9" above the mattress top with a 6"-tall divider behind it (bungee the small gap between the divider's top and the headboard's open edge), carrying the half-round edging lip, Power strip 1 (phones/glasses/flashlights/USB), and the roll bubble level — while the remaining 10.5" of the top tier faces the kitchen. It still comes off Panel C separately — unbolt the 4 base bolts and the 2 brace arms (wing nuts, no tools) — so its own height clears the liftgate opening on its own. See the Headboard Storage Detail render and Section 6, Component 1.
- **One continuous sleeping deck**, 46" wide x 94" long (Panels A + B + C, on uniform legs, flush with the front seatbacks — no gap), deck surface sitting 19.25" above the van floor (17" legs + 1.5" frame rail + 3/4" ply). `leg_height` is driven by the fridge's 15.53" height (see below), not by the folded 3rd-row well depth — confirm 17" clears your actual well before cutting.
...
|---|---|---|---|
| Headboard/pantry (mounted on Panel C, tailgate end) | 14" (of Panel C's own 36") | 46" | **No frame/legs of its own** — bolted to Panel C's deck (4x 1/4-20 + T-nuts) with 2 steel L-angle sway braces down to Panel C's own side rails, rising 22" above deck level; 3 full-width shelf tiers, no top (unbolts separately) — full-depth food storage below, personal nook facing the mattress in the top tier. |
...
| Kitchen unit (JAGAHAHA, in Panel C's void) | 26" deep (closed) | 20" wide | 11.8" tall; flush to Panel C's RIGHT (passenger) edge — shelves swing out that side; own built-in tailgate slide |

**Drawer dimensions** (identical on Panel A and Panel B — 4 total, left + right on each; Panel C has none):
...
| Height | 14.5" | Inside the 17" leg-height storage bay, under the fixed deck |
...
| Panel C — front (B-facing) | **The ONE wall**: 1/2" ply, 46" x 17" | Mounts the 120mm intake fan and passes the fridge DC line (1" grommet) — every hole dimensioned in the Panel C Front Wall render. |
...
|---|---|---|---|---|
| Kitchen unit (JAGAHAHA) | C | 26–46 | 0–26 | 0–11.8 |
| Fridge (BougeRV Rocky 40) | C | 0.5–18.2 | 0–28.74 | 0–16.29 (incl. tray) |
| Intake fan (120mm) | C | 9.4 | 28.74 | 8.3 |
| Exhaust fan (120mm) | C | 18.2 (fridge's kitchen-facing wall) | ~14.4 | 8.3 |
| NTC temp sensor | C | ~17.2 | ~12.9 | 8.3 |
...
| Center divider | A, B | 22.25–23.75 | 1.5–27.5 | 0–17 |
| Headboard/pantry side panels (x2) | C | 0–0.75, 45.25–46 | 0–14 | 19.25–41.25 |
| Headboard/pantry center divider | C | 0–46 | 10.5–11.25 | 19.25–41.25 |
| Headboard/pantry full-depth shelves (x2) | C | 0–46 | 0–14 (full depth) | 25.75, 32.25 |
| Headboard/pantry base cleats (bolt-down, x2) | C | 0–46 | 5.5–8.5 | 19.25 (shelving floor) |
...
| Headboard/pantry personal nook (top shelf's mattress-facing strip) + nook divider | C | 0–46 | 11.25–14 (nook 2.75" deep) | nook surface 33.0 (9" above the mattress top); divider 33.75–39.75 (6" tall) |
| Power strip 1 (relocated, phone/light/Claymore fan) | C | ~31 | on the personal nook, Y~11.25-14 | ~33.8, on the nook surface |
...
- **Panel C**: no drawers at all. Its entire under-deck void is split between the fridge (right side, on its own slide) and the kitchen unit (left side, on its own slide) — both bought products, both accessed by pulling them out through the **open tailgate**, not a side door. There's no "storage bay" here in the drawer sense; the fridge and kitchen units *are* the storage, permanently installed rather than removable containers.
...
| Headboard/pantry full-depth shelves | 2 | 46" x 14" (they're the carcass webs — the old full-height divider is gone) |
| Headboard/pantry base cleats | 2 | 46" x 3" (plain rip — bolted flat to the deck, no bevel) |
| Kitchen drawer hanging cheeks | 2 | 26" x 6.2" (screwed up into Panel C's deck, flanking the drawer) |

**1/2" sheet 1** — 1 drawer box (Panel A right — Panel A's left bay is WAVE 3 open storage, no box, and Panel B has no drawers at all), the fridge's tray, the kitchen drawer's box, Panel C's front wall, and the headboard/pantry's nook divider, ~26 sq ft of material against ~32 sq ft of usable sheet area (the bed platform is 1x4 board lumber — Section 4 — not cut from this sheet):
...
| Headboard/pantry nook divider | 1 | 46" x 6" (top tier only, 2.75" from the mattress face) |
...
| Bottom rails (cube frames) | 5x 46" + 2x 26" | see notes | underside at 2": Panel A both END faces (2x46), Panel B all 4 faces (2x46 + 2x26), Panel C FRONT face only (1x46) — 2x 2" screws + glue into each leg |
...
| Cable grommets | 1" diameter, 9 needed (2 cooktop line, 2 Power strip 1's own line, 3 fridge DC line, 2 DELTA 3 AC charging line — see Section 5) |
...
| Headboard/pantry bolt-down base | 2 base cleats 46" x 3" (3/4" ply, screwed to the shelving's underside) + 4x 1/4-20 x 2" hex bolts w/ fender washers through the cleats and Panel C's deck into T-nuts underneath — solid, rattle-free, and removable with one wrench |
...
| Headboard/pantry netting | Bungee/shock cord (~16ft, 1/4" dia) + 10 small screw-eyes — across the 3 kitchen-face tiers AND the bottom/middle tiers' mattress face (no divider down there anymore); unhook one end to restock |
| Headboard/pantry half-round edging | 1, 3/4" half-round pine trim, 46" long — glued/pinned along the personal shelf's mattress-facing lip |
...
| Headboard/pantry bungee/shock cord + 10 screw-eyes (3 kitchen-face tiers + bottom/middle tiers' bed face) | 1 set | $12 | $12 |
...
| OPTION — electric leveling: 12x compact 12V linear actuators + 3x 4-switch DPDT pods (under Panel A, driver side) + wiring | (option) | ($450–600) | ($0 in total below) |
...
| **BougeRV Rocky 40 (CR04001), 41QT dual-zone** ([bougerv.com Rocky V3.0](https://www.bougerv.com/products/rocky-12v-camp-fridge) — dims verified against the saved user manual: 28.74" x 17.72" x 15.79", 46.8 lb, 80W rated, reversible lid, optional B240 battery) | 1 | $400–500 (UNVERIFIED — check the live listing) | $400–500 |
...
<tr><td>1" cable grommets — need 2 — <strong>PURCHASED</strong> (July 2026)</td><td>EASYEAH 1" rubber grommets, 20-pack ($9.99)</td><td><a href="https://www.amazon.com/Diameter-Hole%EF%BC%8CRubber-Synthetic-Protection-Double-Sided/dp/B085S6S6KH">https://www.amazon.com/Diameter-Hole%EF%BC%8CRubber-Synthetic-Protection-Double-Sided/dp/B085S6S6KH</a></td></tr>
...
The first leg is drawn in crimson on the Headboard Elevations (right-side view): from the strip on the personal nook, the cord is cable-clipped down the passenger side panel's inner face, exits through a 1" grommet in Panel C's deck at the headboard's back corner, crosses under the deck, and leaves Panel C through the front wall's UPPER grommet (3" in from the driver edge, 5.5" up — the fridge DC line uses the lower one). From there it runs the floor channel forward.

Power strip 1 (phone/device charging, reading light, the Claymore fan) mounts on the **headboard/pantry's personal shelf** (Component 1) — which now sits at the **tailgate end of Panel C**, not the front. This flips Power strip 1's cord from the simplest line in the design to one of the longest: it now runs the full length of the platform, crossing the same 2 seams (Panel C → Panel B, Panel B → Panel A) the cooktop and fridge DC lines do. Like the cooktop line, this is a 120V AC cord — household-rated extension cord, not a DC connector.
...
**No longer its own module** — no frame, no legs, no top of its own. This is a shelving superstructure, **bolted down to Panel C's already-built deck** (Component 4) and braced against driving sway by 2 steel L-angle braces tied into Panel C's own side rails, occupying the last 14" of Panel C's own 36" length, at the tailgate end right next to the fridge/kitchen void. **3 full-width tiers, no full-height divider, no top**: two full-depth (14") shelves at 6.5" and 13" split the 22" height into 3 open tiers — bottom and middle are full-depth food storage, netted on both faces; the top tier is split by a 6"-tall nook divider into a 2.75"-deep personal nook facing the mattress (surface 9" above the mattress top, with the half-round edging lip, Power strip 1, and the roll bubble level right on it) and a 10.5"-deep food tier facing the kitchen. It comes off Panel C by removing 4 base bolts and the 2 sway-brace arms (wing nuts at the rail ends) — a few minutes with one wrench — so its own 22" height clears the liftgate opening on its own. Build Panel C (Component 4) first — this component's context is Panel C's own finished frame and top. See the Headboard Storage Detail render for the full dimensioned elevations.
...
- Headboard/pantry nook divider — 1, 46" x 6", 1/2" ply (top tier only — its top stops ~2.25" below the headboard's open edge; bungee that gap)
...
- Bungee/shock cord + 10 screw-eyes (netting: 3 kitchen-face tiers + the bottom/middle tiers' mattress face)
- Power strip 1 + its own dedicated cord run (Component 6)
- Wood screws (1.25" ply-to-ply), wood glue
...
<p class="lego-caption">Build the shelving carcass as its own separate assembly, positioned over the tailgate end of Panel C's deck (shown here in context, but not yet fastened to it): 2 side panels (E) + the 2 FULL-DEPTH shelves (H) glued and screwed in at 6.5" and 13" (2" screws, 2 per shelf end). The shelves ARE the structure — no full-height divider, no top, both faces open. This makes 3 full-width tiers.</p></div>
</div>
<div class="lego-step">
...
<p class="lego-caption">Drop the 6"-tall nook divider (F) into the top tier, 2.75" from the mattress face — screwed down into the top shelf and into both side panels (it stops ~2.25" below the open top edge; run a bungee across that gap too). Then add screw-eyes and stretch bungee/shock cord across every kitchen-face tier AND across the bottom/middle tiers' mattress face: with no divider below the top shelf, the bungees are what keep braking from sliding food onto the bed. Unhook one end to restock, reach through otherwise.</p></div>
</div>
<div class="lego-step">
...
<p class="lego-caption">Fit out the personal nook — the top shelf's mattress-facing 2.75", its surface 9" above the mattress top, a comfortable reach while lying down. Glue/pin half-round edging along the shelf's front lip, mount Power strip 1 on the nook (Component 6 for its dedicated cord run), and screw the ROLL bubble level to the nook beside it — read it from the bed or side door while turning the leg-foot knobs.</p></div>
</div>
</div>
...
<p class="lego-caption">Build the frame: 2 side rails (B) + 2 end rails (A) joined with corner brackets and 2" screws, then the 4 legs (C) — CUT to 16", drilled in the bottom end grain (1/2" dia x 3/4" deep, centered) for a 3/8-16 threaded insert, so the leveling foot brings each leg back to an effective 17" (Leg Leveling Foot Detail render). Much easier to drill before assembly. Legs inset 2.5" from the deck's side edges to clear the floor-level vent intrusion. Then close the bottom: 2 END-face bottom rails (K, 46", underside 2" up — Panel A's SIDE faces stay open so the drawer and WAVE 3 can exit) plus 2 diagonal corner braces up top — the part-cube racks far less than rails + brackets alone. (Panel B's frame differs only at the bottom rails: it closes all 4 faces.)</p></div>
</div>
<div class="lego-step">
<div class="lego-num">2</div>
...
<p class="lego-caption">Build the right (DELTA 3) drawer box (E, 5 pieces, open top) and slide it onto its pair of 20" full-extension slides (F) — one slide side on the frame's outer rail, the other on the center divider. Add a catch (G) so it doesn't slide open in transit. On the LEFT bay, skip the drawer entirely: glue/screw down the 2 glide strips (H) along the floor instead — that's the WAVE 3's storage spot (Component 8).</p></div>
</div>
<div class="lego-step">
<div class="lego-num">4</div>
...
<p class="lego-caption">Build the frame — like Panel A's step 1: side rails (B), end rails (A), corner brackets, 4 legs (C) inset 2.5" from the deck edges (drilled for their leveling inserts BEFORE assembly), the 2 diagonal corner braces, and bottom rails on ALL FOUR faces (2x 46" + 2x 26", underside 2" up) — **the full cube**: nothing exits Panel B sideways, so every face can close, and this is the stiffest frame of the three. That's the entire panel — no divider, no drawers, nothing else. Drill the alignment-pin holes when mating it to its neighbors (Component 5).</p></div>
</div>
</div>
...
- Corner brackets — 4
- Front wall — 1, 46" x 17", 1/2" ply, with the 120mm fan hole + two 1" grommet holes pre-cut (Panel C Front Wall render — every hole dimensioned)
...
<p class="lego-caption">Cut the front wall and its three holes to the render's dimensions — the 4.75" (120mm) fan hole centered on the fridge bay (11.36" from the driver edge, 8.4" up) and two 1" grommet holes at 3" in: fridge DC line at 3" up, Power strip 1's line at 5.5" up — then screw it to the front (B-facing) face: 2x #8 x 1-1/4" into each front leg + 2 into the top rail + 2 into the bottom rail (8 total). This is the ONLY wall on any panel; the intake fan bolts over the fan hole in Component 7. Panel C's sides stay open and its tailgate face is fully occupied by the fridge, cabinet door, kitchen unit, and kitchen drawer.</p></div>
</div>
</div>

### Component 5: Anti-Rattle Bumpers & Alignment Pins
...
- Cable grommets — 9, 1" diameter (2 cooktop line, 2 Power strip 1 line, 3 fridge DC line, 2 DELTA 3 charging line)
...
- Utility cabinet door hardware — 2 small hinges + 1 magnetic catch
...
<p class="lego-caption">Bolt the intake fan over the front wall's fan hole (Component 4 step 3 — blowing IN), install the exhaust fan + NTC sensor (kitchen-facing side of the fridge bay, venting into the utility cabinet), and the control panel enclosure (switches + surge protector) INSIDE the cabinet — just behind the door, screwed to a backer board (the 3/4" sheet's offcut) hung from the deck underside, so opening the door reaches every switch. Wire everything per the schematic. The CO monitor and fire extinguisher are owner-placed — position them yourself once the build is in the van (the CO monitor stays battery-powered and unwired either way).</p></div>
</div>
<div class="lego-step">
<div class="lego-num">5</div>
<div class="lego-parts"><p class="lego-noparts">cabinet door + 2 hinges + magnetic catch</p></div>
<div class="lego-main">
<p class="lego-caption">Build and hang the utility cabinet door over the gap between the kitchen unit and the fridge — 2 small hinges on the kitchen-side edge, magnetic catch on the fridge-side edge.</p></div>
</div>
<div class="lego-step">
<div class="lego-num">6</div>
...
<div class="lego-main">
<p class="lego-caption">Slide the WAVE 3 into Panel A's left bay, resting on the 2 glide strips — no box, no slide hardware, just reach in through the driver's side door. Mount the hose/cord hook inside the utility cabinet (kitchen-side wall) for when the unit is carried off to run at camp.</p></div>
</div>
<div class="lego-step">
<div class="lego-num">3</div>
...
| Location | Purpose |
|---|---|
| Power strip 1 — mounted on the headboard/pantry's personal shelf (relocated from Panel A — Component 1/6) | Phone/device charging, reading light, and the **Claymore V600+ tripod fan** — easy reach from the sleeping area, right by where your head/torso would be |
| Power strip 2 — mounted at the kitchen unit | Powers the induction cooktop and any other small kitchen appliance (kettle, blender) |
...
| Module | Est. weight |
|---|---|
| Headboard/pantry shelving superstructure (side panels + 2 full-depth shelves + nook divider + base cleats, mounted on Panel C's deck) | ~43 lb |
| Panel A (incl. divider + 1 drawer box/slide + corner braces + 2 bottom rails + glide strips, no top) | ~42 lb |
...
| BougeRV Rocky 40 fridge (real product weight, empty) | 46.8 lb (realistically 90-120 lb loaded — why it's on a 200lb-rated slide, not a lift-out module) |
...
**Panel B is still no longer an easy one-person lift, even after losing its top.** The bigger drawers this redesign needed (25" deep x 14.5" tall, up from 22"x8.5") add roughly 45-50 lb of drawer-box plywood alone to a fully-drawered panel — dropping the top (~23 lb) brings Panel B down from ~85 lb to ~62 lb, but that's still past the ~50 lb rule of thumb for a comfortable solo carry. **Plan on a two-person lift for Panel B**, or consider lighter drawer material (thinner ply, or aluminum-composite panel instead of 1/2" Baltic birch) if solo lift-out matters to you. **Panel A, by contrast, is back under that threshold (~39 lb)** now that its left bay lost a whole drawer box to the WAVE 3's open storage — one less 25"x14.5" box's worth of 1/2" ply and one less slide pair. Panel C also got *lighter* than before (~38 lb) now that it carries no drawers of its own — just the frame, deck, fridge tray, and a small control enclosure (the headboard/pantry's ~48 lb shelving rides on top of this when in place, but detaches separately). **The headboard/pantry's own ~48 lb is still a one-person lift, but it's no longer comfortably under the ~50 lb rule of thumb the way it was at its old, shallower 8" depth (~35 lb)** — growing the food compartment to 10.5" deep (Section 1's depth-split decision) added a real amount of plywood, and that's before counting whatever food is actually stocked on the shelves. Empty the food compartment before lifting it — with cans/boxes loaded, this is no longer a light lift at all.

**Fit and quiet-in-transit checklist:**

- Hand-hold holes (Section 3) are sized and placed for a two-hand, centered lift on each panel.
- Bumper strips + alignment pins (hardware in Section 3, installed in Section 6 Step 7) go at the Panel A/B and Panel B/C seams — this is what stops the panels from rubbing or rattling against each other on the road, and keeps them from creeping out of position.
- Do the full lift-out/reinstall dry run in Step 13 before you consider the build done — that's the real test of whether the modular fit is tight enough to stay quiet but loose enough to lift easily (with a second person, given the weight note above).
