...
### The rear floor under Panel C — is there even floor to bolt to? (UNVERIFIED — check BEFORE buying anchors or drilling anything)

**The problem (owner, July 2026, from a photo of the actual cargo area):** the rearmost stretch of the Sienna — exactly where Panel C, the fridge slide, and the kitchen unit live — is not plain sheet-metal floor. It's the 3rd-row stowage area: a **deep cargo well** the folded seatbacks drop into, so much of the "floor" back there is actually **folded seatback over a void**, plus the seats' own latch/hinge hardware and (passenger rear corner) the hybrid's 12V battery access panel. The whole E-track plan in Section 8 assumes 8 anchors through-bolted into *structural floor pan* at Panel C's footprint — **nobody has yet confirmed that drillable pan actually exists at those 8 X/Y positions.** A 1000 lb-rated anchor bolted through a seatback, a plastic well liner, or thin trim over a void is worth nothing. This section is what settles it — do it with the 3rd row folded exactly as it will sit in camper mode.

| # | Measure / check | Feeds | Current assumption | Your finding |
|---|---|---|---|---|
| F1 | With the 3rd row folded flat, map the load surface: press/knock along the whole Panel C footprint and mark (masking tape) where it's **folded SEATBACK**, where it's the **well/tub** (hollow underneath), and where it's **carpet over solid pan** | The entire Section 8 anchor plan | assumed all solid pan — UNVERIFIED | |
| F2 | Fore-aft position of the stowage well: where its front and rear edges sit, measured from the closed hatch (same Y datum as measurement #1) | Whether ANY of the 8 anchor X/Ys (fridge install detail, Section 2) land on real pan | — | |
| F3 | Well depth below the surrounding load floor, and its width | Whether the well floor itself (its bottom is structural) could take the anchors instead, on standoffs/spacers | — | |
| F4 | At each of the 8 planned anchor positions specifically: solid pan / well void / seat hardware / liner? | Go–no-go per anchor; relocate any that miss | — | |
| F5 | Where Panel C's 4 legs and the kitchen unit's footprint actually bear: solid floor, or folded seatback (springy — a deck leg on seat cushion won't sit level or solid)? | `leg_height`, deck levelness, kitchen strap-down | — | |
| F6 | Does exposing real pan require REMOVING the 3rd row rather than folding it? If so: does the 3rd row carry its own SRS/seatbelt-pretensioner wiring? | Section 9 covers 2nd-row removal only — a 3rd-row removal procedure (and possibly more emulators) would be NEW scope | assumed fold-only — UNVERIFIED | |
| F7 | Match top-side positions to the underside: crawl under and confirm what's directly below each candidate anchor spot (fuel tank, lines, spare area, exhaust) | Section 8's pre-drill clearance check, done at the SAME 8 spots F4 approves | — | |
...
4. **Close-ups of the seat latch/hinge hardware** anywhere it falls inside Panel C's 36" footprint — this is what an anchor or a deck leg would be fighting for space with.
5. **The passenger rear corner** showing the 12V battery access panel — the one spot already ruled out for anchors (Section 8).
6. **From under the van, the same area looking up** — fuel/brake lines, tank, exhaust, spare — framed so it can be lined up against photo 1's tape datum. This pairs with F7.

**If F1–F4 come back "no solid pan where the anchors want to be,"** the securing plan in Section 8 has to change before anything else proceeds — candidate fallbacks to evaluate *against what you find* (not decided here): move the anchors to wherever pan really exists and bridge to the slide rails with longer steel angle; bolt into the well's bottom with standoff spacers; or use the 3rd row's own seat-mount points (they're the strongest hardpoints in that whole area, engineered for crash loads — but that likely means removing the 3rd row, which triggers F6). Every one of those changes geometry that `params.scad` currently takes on faith, so settle this measurement before cutting Panel C.
...
**3/4" sheet** — now just Panel C's deck (the plywood pantry is gone — Component 1 is prefab drawers). This sheet is mostly spare: rip the rear-pantry hold-down cleats from the offcut, and keep the rest for the control-panel backer board (Section 6) and repairs:
...
| RV leveling blocks, Lynx-style 10-pack + 1 wheel chock (per-site leveling at the WHEELS — see the Block Calculator note, Appendix D) | 1 | $40 | $40 |
...
| 5/16" stainless carriage bolts + nylon-insert lock nuts + fender washers (2 per E-track anchor, through the floor pan) | 16 | — | $12 |
| Blue medium-strength threadlocker (Loctite 242 or equivalent — on the floor-pan bolts, in addition to the lock nuts) | 1 bottle | $8 | $8 |
| Zinc-rich cold-galvanizing compound, brush-on or spray (bare cut-edge rustproofing at all 16 drilled holes, before sealant) | 1 can | $12 | $12 |
| Rubberized undercoating spray (2nd rustproofing layer over the anchor footprints, underside) | 1 can | $10 | $10 |
...
- **IRIS USA 12"W stackable storage drawers** (Home Depot model #500163, gray, sold as 3-packs at ~$30.29 — buy 2 packs) — **use 4 units**, 12.1" W × 14.3" D × 8.4" H each, stacked 2 wide × 2 high (2 spares for swap-in). The 14.3" depth sits 0.3" proud of the deck edge — inside the 2" hatch reserve
...
- Flush-mount deck D-rings — 2 (or a short E-track offcut), screwed to the deck flanking the cluster
...
**Before you drill anything — actually, before you even buy the anchors**: it is NOT yet verified that drillable structural floor pan exists at these 8 positions at all. The rearmost stretch of the Sienna is the 3rd-row stowage area — a deep well the folded seatbacks drop into — so much of the surface under Panel C's footprint may be folded seatback or well liner over a void, not sheet metal. **Work through the "rear floor under Panel C" checklist at the bottom of Section 0 (F1–F7) first**; if solid pan isn't where these anchors want to be, the securing plan changes before anything else proceeds. Then, at the spots that checklist approves: the E-track bolts go all the way through the van's floor pan. Get underneath the van and physically confirm each of the 8 locations (see the fridge install detail diagram, Section 2, for exact X/Y) is clear of the fuel tank, fuel lines, brake lines, and wiring harnesses before drilling — reposition an anchor if it isn't. Seal each hole with butyl or silicone sealant on both sides of the bolt to prevent water intrusion and rust. **And keep the passenger rear corner clear: the hybrid's 12V battery access panel lives there (directly under the rear AC outlet) — no anchor over it.**

**Warranty note (not legal advice — verify against your own paperwork before drilling)**: drilling 16 permanent holes through the factory floor pan is a real, non-reversible modification, and it's worth understanding what it does and doesn't put at risk before you commit. In the US, the Magnuson-Moss Warranty Act prevents a manufacturer or dealer from voiding your *entire* vehicle warranty just because you added an aftermarket modification elsewhere — they can only deny a specific claim if they can show the modification actually caused that specific failure. So drilling into the floor pan at the rear cargo area is very unlikely to void coverage on, say, the engine, transmission, or infotainment system. **What it most likely DOES affect is the corrosion/rust-perforation warranty specifically** — most manufacturers' corrosion warranties cover rust-through of factory-sealed body panels, and a self-drilled, non-factory penetration in that same panel is exactly the kind of thing a dealer can point to if rust later develops at or near that hole. If the van is still under a corrosion warranty you care about, call your dealer's service department (not sales) and ask directly how they'd treat an owner-drilled, sealed E-track anchor hole in the cargo floor before you drill — get the answer in writing/email if you can, since "the tech at the counter said" isn't worth much if a claim is denied later.

**Drill spec for the 16 floor-pan holes (2 per anchor × 8 anchors)**: the bolt is 5/16" (0.3125"), but don't chuck a 5/16" bit straight into sheet steel — it will walk on the crowned/curved floor pan surface and the hole will end up off-center from the anchor's own mounting slots. Step drill instead:

1. **Center-punch** each hole location first (mark through the anchor's own mounting holes once it's positioned and clamped/taped in place) — this alone stops most bit walk.
2. **Pilot with a 1/8" bit**, then step up to **3/16"**, then to the final **11/32" (0.344") clearance hole** — one size over the 5/16" bolt shank, not an exact match. A carriage bolt through sheet metal (unlike into wood) has nothing for its square neck to bite into, so there's no anti-rotation benefit to a tight hole; the extra 1/32" just gives you slop to align the floor-side washer/nut without fighting the hole, which matters more here since you're working blind, on your back, underneath the van.
3. **Use cutting oil (or equivalent) on every pass** — the floor pan is sheet steel, not wood; drilling dry work-hardens the edge and dulls bits fast, and a lubricated bit cuts a cleaner, less-burred hole.
4. **Deburr both faces** (top and underside) with a deburring tool or a larger bit hand-turned over the hole — burrs left in place will cut into the butyl/silicone seal and give water/rust a head start exactly where you're trying to seal it out.
5. **Test-fit the bolt through both layers dry** (no sealant yet) before you commit — confirm the anchor sits flush and the bolt isn't binding, then remove it, apply sealant per the step above, and do the final bolt-up.

This step-drilling sequence applies to all 16 holes (both fridge rail lines and all 4 kitchen-unit corners) — the floor pan gauge doesn't vary enough across those 8 locations to change the approach.

**Rustproofing the holes — the sealant alone isn't the whole job.** A freshly-drilled edge is bare steel with any factory coating (paint, e-coat, galvanization) cut straight through — that raw edge, not the surrounding painted floor, is where rust actually starts. Before any sealant goes on:

1. **Degrease before you treat anything** — the cutting oil from the drill-spec steps above is still on and around the hole, and no primer, galvanizing compound, or sealant bonds well over oil residue. Wipe the whole area (both faces) down with brake cleaner or a wax-and-grease remover and let it flash dry first.
2. **Treat the bare cut edge**, on both the top (inside the van) and underside faces — a zinc-rich cold-galvanizing compound (brush-on or spray, e.g. a ZRC-type or Rust-Oleum Cold Galvanizing Compound — any COLD-GALV product labeled for bare/cut steel edges works) applied right onto the exposed metal ring around each hole. This is the step that actually stops corrosion from starting; sealant alone just keeps water out, it doesn't replace the coating you cut through. **Let it cure fully per the can's label** (typically an hour or more, longer in cold/humid conditions) before doing anything else to the hole.
3. **Then butyl (not just silicone) as the water barrier** — butyl tape/sealant stays flexible and keeps gripping through the van's temperature swings and road vibration better than plain silicone, which can harden and crack loose from bare metal over a few seasons. Apply it both under the anchor's mounting plate (as a gasket, so water can't wick in sideways under the plate) and around the bolt shank itself on both faces.
4. **Follow up with a rubberized undercoating spray** over the whole repaired area from underneath, once the sealant has cured — this is the same product/approach used for stone-chip and rust protection on factory floor pans, and it gives a second sacrificial layer over the anchor's footprint, not just the bolt holes themselves. Mask off the anchor's own bolt heads/nuts first if you want them serviceable later without scraping through overspray.
5. **Re-inspect yearly**, especially after driving through winter road salt — surface rust starting at one of these 16 points is far easier to catch and treat early than after it's spread under the anchor plate. The **underside** is the priority check: it's the face actually exposed to road spray and salt, while the topside (inside the van, under the anchor) mostly just needs to stay dry.

**Getting underneath, and actually threading the nuts on** (this is the part most first-time installers underestimate): drive the van onto a rated pair of automotive ramps on flat, hard ground, set the parking brake, and chock a wheel that's still on the ground — don't rely on a floor jack alone as your only support while you're underneath it; add jack stands at the Sienna's specified lift points (owner's manual) if you need more working clearance than the ramps give.

The nut-threading problem: a 5/16" **carriage bolt** is designed so its square shoulder (just under the domed head) bites into a squared pilot hole in *wood*, stopping it from spinning while you tighten the nut from the other side — but the anchor is going into **sheet metal**, where the round clearance hole gives that square shoulder nothing to grip. Unless the Snap-Loc anchor's own mounting plate has a stamped hex/square recess that captures the bolt head (check this against the actual product before you buy, not assumed here), the bolt will just spin in place the moment you try to run a nut up from underneath. Three ways to actually get it done:

- **Two-person (simplest)**: one person holds the bolt head steady from inside the van with a wrench or locking pliers on the shank just under the head, while the other threads the nut on from underneath with a socket and ratchet.
- **Solo, using friction**: drop the bolt through from above so the anchor plate's own weight roughly seats it, then from underneath, thread the nut on by hand first with light upward pressure — that's usually enough to keep the bolt from spinning for the first several turns; once a few threads are engaged it self-jams enough to finish with a ratchet.
- **Use a nylon-insert lock nut, not a plain nut, either way** — the added friction means once it's hand-tight, it won't back out or spin loose later from the van's own road vibration, which matters here since you can't easily re-check these 16 nuts once the panels and appliances are installed over them.

**Add threadlocker on top of the lock nut, not instead of it** — belt-and-suspenders is worth it for 16 fasteners you won't be able to see or reach again without pulling Panel C, the fridge tray, or the kitchen unit back out. Use **blue, medium-strength threadlocker (Loctite 242 or equivalent)**, not red: red (Loctite 271-class) is rated near-permanent and typically needs a heat gun to break the bond for removal, and heat plus "underneath the van, near the fuel tank/lines you already had to check clearance around" is exactly the combination to avoid. Blue holds fine against vibration and still lets you break a nut free with a wrench if you ever need to unstrap/service the fridge slide or the kitchen unit's E-track mounts.

**One real interaction to know about**: threadlocker cures anaerobically — it needs the threads clean, dry, and oil-free to set up at all, so the cutting oil from drilling (and any residue left on the bolt itself) has to be wiped off with brake cleaner or a similar degreaser immediately before you apply it, same as the degreasing step above for the rustproofing. Apply a drop or two to the bolt's threads right before threading the nut on (working time is only a few minutes on most blue threadlockers), then let it cure the label's stated time — usually 24 hours — before you put full load on that anchor (i.e., before you strap the fridge/kitchen down and drive on it).

Final torque is snug plus a firm quarter-turn past hand-tight — a 5/16" bolt into thin floor-pan steel strips more easily than it under-tightens, so don't chase it with a long breaker bar.
...
| Panel C (frame + front bottom rail + deck + front wall + kitchen drawer & cheeks + control enclosure — the fridge TRAY + slides stay bolted to the van floor, and the fridge/kitchen units are tracked separately below) | ~61 lb — the heavy one (unclip the kitchen drawer box from its slides first: ~48 lb) |
