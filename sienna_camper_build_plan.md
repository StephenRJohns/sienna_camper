...
**The problem (owner, July 2026, from a photo of the actual cargo area):** the rearmost stretch of the Sienna — exactly where Panel C, the fridge slide, and the kitchen unit live — is not plain sheet-metal floor. It's the 3rd-row stowage area: a **deep cargo well** the folded seatbacks drop into, so much of the "floor" back there is actually **folded seatback over a void**, plus the seats' own latch/hinge hardware and (passenger rear corner) the hybrid's 12V battery access panel. Section 8's no-drill securing design — an anchor board bearing on this surface, tongues butting the step at the striker row, straps into the 3 striker loops — needs the same geography mapped before its outline can be cut: where the surface is solid, where the well and its edges are, exactly where the strikers sit, and what Panel C's legs land on. Do the whole survey with the 3rd row folded exactly as it will sit in camper mode. (The old version of this checklist gated 16 drilled floor-pan holes; the drilling is gone, and with it the under-van fuel/brake-line clearance crawl.)

| # | Measure / check | Feeds | Current assumption | Your finding |
|---|---|---|---|---|
| F1 | With the 3rd row folded flat, map the load surface: press/knock along the whole Panel C footprint and mark (masking tape) where it's **folded SEATBACK**, where it's the **well/tub** (hollow underneath), and where it's **carpet over solid pan** | Where the anchor board's strips can bear solidly, and where they'd need filler blocking | assumed usable as-is — UNVERIFIED | |
| F2 | Fore-aft position of the stowage well: where its front and rear edges sit, measured from the closed hatch (same Y datum as measurement #1) | The board's strip lengths, and where filler blocking starts/stops | — | |
| F3 | Well depth below the surrounding load floor, and its width | Sizing the filler blocking that carries a board strip across the well down to its structural bottom | — | |
| F4 | The 3rd-row striker row: each loop's fore-aft distance from the hatch and lateral position, loop inside clearance (strap hook must seat), and whether the FOLDED seatbacks leave all 3 loops exposed and hookable | Section 8's striker straps + the steel tongues' length (assumed striker row ~46–50" from the hatch → ~10–14" tongues) — the strikers are confirmed PRESENT (photo, July 2026) but not yet measured | ~46–50" from hatch — UNVERIFIED | |
...
| F6 | Does a solid bearing surface require REMOVING the 3rd row rather than folding it? If so: does the 3rd row carry its own SRS/seatbelt-pretensioner wiring? | Section 9 covers 2nd-row removal only — a 3rd-row removal procedure (and possibly more emulators) would be NEW scope. Removal would also expose the 3rd row's own seat-mount bolts as bolt-in anchor-plate hardpoints (no new holes) — the strongest points in the zone if the striker plan needs help | assumed fold-only — UNVERIFIED | |
| F7 | The step/riser at the striker row: is there a square, hard vertical face (metal under the trim?) for Section 8's two steel tongues to butt against, and how tall is it? | The forward (compression) load path — if the step is soft trim over air, the tongues need a different bearing target (e.g. the well's forward wall, F2/F3) | assumed hard step at the striker row — UNVERIFIED | |
...
**If the survey comes back ugly** — loops buried under the folded seatbacks, no hard face at the step, nothing solid for the strips to bear on — the Section 8 design flexes rather than dies: filler blocking down to the well's structural bottom (F3), the well's forward wall as the tongues' bearing target (F7), or the 3rd row's own seat-mount bolts with anchor plates (F6 — likely means removing the 3rd row, which is new Section 9-style scope). Every one of those changes geometry that `params.scad` currently takes on faith, so settle this survey before cutting Panel C or the anchor board.
...
The fridge's slide mechanism itself, closed vs. fully extended. The VADANIA rails stand vertically BESIDE the tray (side-mount): each fixed rail on a steel riser angle bolted to the anchor board (Section 8 — no-drill; **this render and the install detail above still draw the superseded E-track floor anchors** until the Section 0 floor survey fixes the board's geometry and the SCAD files get updated), each moving rail on the tray's 1x3 side apron — nothing under the hanging tray, so the slide hardware adds zero height and the mounted fridge keeps 0.33" of running clearance under Panel C's end rail:
...
| Steel riser angles for the fridge slides, ~2"x2"x3/16" x 24" (1 per fixed rail, drilled to match — bolted atop the anchor board's rail-line strips with 1/4-20 screws into T-nuts, the rail screws to the vertical face 3/4" lower than the old floor-bolted spec, Section 8) | 2 | $12 | $24 |
...
| Anchor board — 3/4" ply H/comb (bridge + 4 strips, Section 8; cut from the 3/4" sheet's spare area if it fits, else a 2'x4' project panel ~$25) + 1/4-20 machine screws & T-nuts ~30 ea | 1 | $20 | $20–45 |
| Steel flat bar 2" x 3/16" (the 2 forward bearing tongues, cut to the F4-measured length) | 1x 36" bar | $15 | $15 |
...
- Anchor board (3/4" ply bridge + 4 strips, Section 8) + 2 steel bearing tongues (2" x 3/16" flat bar) + non-slip mat + 1/4-20 machine screws & T-nuts
...
<div class="lego-main">
<p class="lego-caption"><strong>Before cutting the anchor board</strong>, finish the Section 0 floor survey (F1–F7): confirm all 3 striker loops are exposed and hookable with the 3rd row folded, the step at the striker row gives the tongues a hard face, and the board's strips land on solid surface (filler blocking where they don't). ALSO: the hybrid's 12V battery access panel sits at the passenger rear corner (under the rear AC outlet) — the kitchen-side strip stops short of it so the panel stays serviceable. Nothing gets drilled into the van at any point in this component.</p></div>
</div>
<div class="lego-step">
<div class="lego-num">2</div>
...
<div class="lego-main"><img src="renders/fridge-install-detail.png" alt="Fridge install anchor positions (render still shows the superseded E-track anchors)">
<p class="lego-caption">Bench-build the anchor board (Section 8): cut the bridge + 4 strips, counterbore and seat the T-nuts flush from below, bolt on the riser angles, L-track lengths, bridge D-ring fittings, and the 2 steel tongues (blue threadlocker on every screw; cure ~24h). Set it in the van on its non-slip mat, tongues butted against the step at the striker row, notch Panel B's rear bottom rail where the tongues/straps cross under it, then hook and tension the 3 ratchet straps from the bridge D-rings into the 3rd-row striker loops. No holes in the van — the render above still shows the old E-track anchors until the survey-driven redraw.</p></div>
</div>
<div class="lego-step">
<div class="lego-num">3</div>
...
**What carries the load instead — the 3rd-row strikers.** At the front edge of the folded-3rd-row area sit **3 crash-rated latch loops**: the strikers the 3rd-row seat legs lock into when the seats are deployed (photographed and confirmed present, July 2026). They're engineered to hold an occupied bench seat in a collision — FMVSS 207-class loads — which makes them the strongest exposed steel anywhere in the cargo floor, and with the 3rd row folded they sit empty. The fridge and kitchen live at the tailgate end, directly over the 3rd-row stowage well, so these loops are also the **nearest** rated hardpoints to the mass they restrain. The other candidate hardpoints — the 2nd-row seat mounts that Section 9's seat removal vacates — sit 2+ feet further forward, under Panels A/B; reaching them from the appliances would mean running straps under Panel B's entire footprint. They stay in reserve (bolt-in anchor plates under the factory seat bolts — still no new holes) in case the latched panel train ever wants its own tie-down, but the appliances don't use them.
...
- **Four strips running rearward from the bridge**: one under each fridge fixed-rail riser line (the driver edge, and the center/cabinet gap), and one along each side of the kitchen unit. The strips stop ~2" short of the tailgate edge so Panel C's rear corner legs keep standing on the bare van floor — **Panel C's lift-out design is untouched, and it still never structurally touches the fridge or its slide.**
- **Nothing under the fridge tray or under the kitchen unit.** The tray keeps hanging between the slide rails at its 0.5" floor clearance and the kitchen sits on the van floor as before — the board occupies only the strips beside them, so `leg_height`, the 0.33" end-rail clearance, and the kitchen-drawer stack are all unchanged.
- The **fridge riser angles bolt on top of the rail-line strips** (1/4-20 machine screws into T-nuts from below). The risers now stand 3/4" higher than the old floor-bolted spec, so drill their rail holes 3/4" lower — the riser holes were always "drilled to match" (BOM), and the rail itself keeps the same absolute height: the tray and fridge don't move.
...
1. **Forward (the panic-stop direction, and the big one) — hard bearing, not straps**: two **2" x 3/16" steel flat-bar tongues** bolt under the bridge and run forward along the floor to **butt the raised step at the striker row**, so a hard stop drives the whole board into vehicle structure, wood-and-steel against steel. (A tension strap to a *forward* anchor goes slack the moment the load moves forward — that's why this path is compression.) Each tongue is good for well over 1,500 lb in column before buckling; two of them against ~155 lb of loaded cargo-plus-board is a wider margin than the old E-track plan had.
...
**Why this margin is enough** — same DOT yardstick the old plan used: commercial cargo securement rules (49 CFR 393, Subpart I) size tie-downs for 0.435g forward / 0.5g rearward / 0.25g lateral deceleration, with aggregate working load at least 50% of the cargo's weight. Cargo here is ~135 lb loaded (fridge ~90 lb + kitchen 45 lb) plus ~20 lb of board — the 50% rule asks for ~80 lb of aggregate WLL. The 3 striker straps alone provide **1,200 lb, 15x that**, before counting the forward bearing path, which isn't strap-limited at all. Every link in the chain is rated: straps 400 lb WLL each, stud fittings 1,333 lb, L-track ≥2,000 lb/ft, and the striker loops themselves are seat-crash hardware rated far beyond any of it — the straps govern, exactly where overbuilding is cheapest. (A DIY conversion isn't held to DOT's certification process; the baseline is used as an engineering floor, not a legal one.)

**Gate: the Section 0 floor survey (F1–F7) still comes first — it just answers different questions now.** Nothing gets drilled anymore, but the board's outline can't be cut until the survey pins down: where the load surface is solid vs folded seatback vs well void (F1, F4 — a board strip bearing on springy seatback needs filler blocking down to the well's structural bottom, per F3's depth), the strikers' actual fore-aft position and whether the folded seatbacks leave all 3 loops exposed and hookable (F4), and whether the step at the striker row gives the tongues a square face to bear on (F7). The strikers are confirmed to EXIST (photo, July 2026); their position — assumed ~46–50" forward of the hatch, which puts the tongue run at ~10–14" — is not yet measured. `params.scad` and the install renders wait on the same survey, so **the fridge-install detail render still draws the superseded E-track floor anchors until then**. **And keep the passenger rear corner clear: the hybrid's 12V battery access panel lives there (directly under the rear AC outlet) — the board's kitchen-side strip stops short of it, and battery service stays "unstrap + slide the kitchen out."**
...
1. **Bench-build the board first**: cut the H/comb outline (final dimensions only after the F1–F7 survey), counterbore the underside and seat the 1/4-20 T-nuts flush, then bolt on the riser angles, the L-track lengths, the bridge's stud-fitting D-rings, and the two steel tongues. A drop of **blue medium-strength threadlocker (Loctite 242 or equivalent)** on each machine screw — these fasteners hide under the appliances, so they should never need re-checking. Let it cure per the label (~24h) before loading.
2. **Set the board in the van** on its non-slip mat, tongues forward until they butt the step at the striker row, and notch Panel B's rear bottom rail where the tongues and straps cross under it.
...
5. **Nothing to rustproof, seal, or crawl under the van for** — no holes were cut, so the old yearly rust inspection is gone too. To return the van to stock: release 7 straps, unbolt nothing from the vehicle, and lift the board out with its hardware. Zero evidence it was ever there.
...
2. Bench-build the no-drill anchor board (bridge + strips + risers + L-track + tongues, Section 8), then set it in the van on its mat and tension the 3 striker straps — **the board's final outline needs the Section 0 floor survey (F1–F7) done first**; nothing is drilled into the van. *Verify strap tension after the first drive.* **The hybrid's 12V battery access panel is at the passenger rear corner (under the rear outlet) — the kitchen-side strip stops short of it, and it stays reachable by unstrapping/sliding the kitchen out.**
...
**Anchoring (no-drill, Section 8)** — L-track 48" 4-pack ($95); stud fittings w/ D-ring 10-pack ($45); Erickson ratchet straps, 2 4-packs ($64); anchor-board ply + 1/4-20 screws/T-nuts ($20–45); steel flat bar for the 2 tongues ($15); non-slip mat ($25); fridge-slide steel riser angles ×2 ($24) + 1x3 apron stock ($6).
...
| **NET permanent weight added vs. stock** | **≈ 344 lb** |

So the van, converted and empty, weighs only about **344 lb more than a stock seatless curb** — and because the seats were part of the factory curb weight, pulling them **frees ~120 lb of payload** for your gear. **Weigh your actual seats to confirm** — captain's chairs vary; 120 lb is an estimate.

**Loaded scenarios** (payload = everything you add to the van — build + people + cargo):

| Scenario | Weight |
|---|--:|
| Net build (added − seats removed) | ≈ 344 lb |
| + provisions (food, water, cookware, bedding ~120 lb) | ≈ 464 lb |
| + 2 occupants (~340 lb, in the front seats while driving) | ≈ 804 lb |

#### Weight distribution

- **Fore–aft:** the build's own center of mass sits **≈ 52.1" back** from the front seatbacks — aft of the 47" geometric center (the prefab pantry and the axle-stowed spare pull it forward; the anchor board pulls it back). Breaking it into thirds: **front 29%** (136 lb — Panel A + the DELTA 3/WAVE 3 stack + the bed platform), **middle 25%** (118 lb — Panel B now carries the spare + totes at the axle), **rear 45%** (210 lb). That rear share is the **tailgate cluster** — the prefab drawer pantry (~15 lb), the fridge (41 lb), the kitchen unit (45 lb), the anchor board (~23 lb), and Panel C's own structure — most of which has to live at the back for slide-out access.
