...
| F7 | The step/riser at the striker row: is there a square, hard vertical face (metal under the trim?) for Section 8's two steel tongues to butt against, and how tall is it? | The FALLBACK forward load path (compression bearing) if F8 rules the rail ends out | assumed hard step at the striker row — UNVERIFIED | |
| F8 | The 2nd-row long-slide floor rails, with the seat carriages parked fully forward: where do the rails' REAR ends sit (distance from the hatch, lateral spacing), and what's at each end — an end cap that unclips, exposed track bolt heads, an open track slot, a lip a clamp plate could grab? Photograph the end hardware close-up | Section 8's PRIMARY forward connection: the anchor board's 2 steel tongues bolt/clamp to these rail ends (no new holes) — this measurement decides the bracket detail and tongue length | assumed the rail ends reach the striker-row zone — UNVERIFIED | |
...
| Anchor board — 3/4" ply H/comb (bridge + 3 strips, Section 8; ONE-piece cut from the 3/4" sheet's spare preferred — else smaller stock joined with glued + screwed 6" half-laps, assembly sheet V5) + 1/4-20 machine screws & T-nuts ~30 ea + wood glue & #8 x 3/4" screws | 1 | $20 | $20–45 |
...
<div class="lego-main">
<p class="lego-caption"><strong>Before cutting the anchor board</strong>, finish the Section 0 floor survey (F1–F8): confirm all 3 striker loops are exposed and hookable with the 3rd row folded, find where the 2nd-row floor rails' rear ends sit and what the tongue brackets can bolt/clamp to there (F8 — the striker-row step is the bearing fallback, F7), and confirm the board's strips land on solid surface (filler blocking where they don't). ALSO: the hybrid's 12V battery access panel sits at the passenger rear corner (under the rear AC outlet) — the kitchen-side strip stops short of it so the panel stays serviceable. Nothing gets drilled into the van at any point in this component.</p></div>
</div>
<div class="lego-step">
<div class="lego-num">2</div>
...
- **How the ply joins (assembly sheet, V5)**: prefer cutting the whole H/comb as **ONE piece** from the 3/4" sheet's spare (46"x36" envelope, most of it falls away as offcut) — bridge and strips continuous, zero joints. If you're cutting from smaller stock instead, join each strip to the bridge with a **6" half-lap**: a 3/8" rabbet in the bridge's underside meeting a matching rabbet in the strip's top face, **glued full-face (Titebond II or better) + 4x #8 x 3/4" screws per joint** driven from above, clear of the riser and L-track bolt patterns. These joints matter: the striker straps hold the *bridge*, and everything bolted to the strips hangs off it in a rearward pull — each lap is loaded in tension, so glue the whole face, not dabs. (The steel tongues are not wood joints — they bolt through the bridge with the 1/4-20 T-nuts.)
...
1. **Bench-build the board first**: cut the H/comb outline (final dimensions only after the F1–F8 survey) — one piece from the sheet spare if it fits, else glued + screwed 6" half-laps join the strips to the bridge (assembly sheet, V5) — then counterbore the underside and seat the 1/4-20 T-nuts flush, then bolt on the riser angles, the L-track lengths, the bridge's stud-fitting D-rings, and the two steel rail tongues (their forward ends drilled/bent to match what F8 found at the rail ends). A drop of **blue medium-strength threadlocker (Loctite 242 or equivalent)** on each machine screw — these fasteners hide under the appliances, so they should never need re-checking. Let it cure per the label (~24h) before loading.
...
2. Bench-build the no-drill anchor board (bridge + strips + risers + L-track + rail tongues, Section 8), then set it in the van on its mat, bolt/clamp the tongues to the 2nd-row floor rails' rear ends, and tension the 3 striker straps — **the board's final outline needs the Section 0 floor survey (F1–F8) done first**; nothing is drilled into the van. *Verify strap tension after the first drive.* **The hybrid's 12V battery access panel is at the passenger rear corner (under the rear outlet) — the kitchen-side strip stops short of it, and it stays reachable by unstrapping/sliding the kitchen out.**
