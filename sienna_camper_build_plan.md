...
| Depth (fore-aft) | 22" | Fits between the panel's front and back legs — the **lapped** legs sit 1.5" inboard at each end, so the bay's clear fore-aft span is 23", not 26" (this row read 25" before the lap). Clear interior 20.5" against the DELTA 3's 15.7" |
...
- **Confirmed exact product** (real listing photos with dimension callouts): **18.1"L x 9.1"W x 4.3"H** (briefcase-style unit with a fold-down handle) — fits the tray with real margin: ~4.9" of length, ~6.6" of width, ~1.4" of height to spare. Two 6.3"-diameter induction zones, **1800W nameplate, which is a COMBINED cap across both zones — not 1800W per burner** (the listing states the design ensures power will not exceed 1800W when both burners are in use; measured single-zone max is ~950–1000W — see the bench test below), plus an included removable non-stick cast-iron griddle pan that lays across both zones — a direct match for "two burner or griddle" in one unit.
...
**What the 1800W rating actually buys you — read this before you blame the pot.** 1800W is the **combined ceiling for the whole unit**, not what one burner delivers. The listing says as much: the design ensures power will not exceed 1800W when both burners are in use. So a single pot never sees 1800W — the bench test measured **~934–1000W at P9 on one zone**, and if the second zone is also running, the unit splits its budget and that pot may be getting **~900W or less**. That is a real cooking difference: ~950W is a strong simmer and a slow, lid-on boil, not a fast rolling boil. Three habits follow from it — **boil on ONE zone with the other zone OFF**, keep a lid on and start with the water you actually need, and expect the griddle (which lays across BOTH zones) to share the same budget rather than double it.

**Open question worth 5 minutes:** the ~1000W single-zone figure was read with the unit's other zone idle but the unit powered — worth re-running one burner at P9 with the second zone fully off, and trying **each** zone in turn, in case the two are not identically rated. If one zone turns out to be the stronger one, that is the boil burner, and it is worth knowing which side of the cooktop it is on before you are cooking in the dark.
