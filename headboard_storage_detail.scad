...
module drawing() {
    ...
    // installed slot ----
    shelf_zs = [headboard_mid_shelf_z, headboard_personal_shelf_z];
    for (i = [0 : 1]) {
    ...
    // the TOP shelf's mattress-facing (near) lip — that 2.75in strip
    // of the top shelf IS the personal nook
    top_y = y0 + pull * 2.2;
    ...
    // ---- nook divider: raised above the open top, drop arrow down
    // to its slot on the top shelf, headboard_shelf_depth back from
    // the mattress face ----
    ...
    cap(str("2 FULL-DEPTH shelves -> 3 full-width tiers; nook divider (4) splits the TOP tier: personal nook ", Yp, "\" (bed side) | food ", Yf, "\" (kitchen side)"), 13, -19, 1.4);
    cap("No full-height divider, NO TOP. The shell bolts down to Panel C's already-built deck (3); 2 L-angle sway braces tie into Panel C's own side rails.", 13, -22, 1.3);
    cap("Empty the food tiers before lifting. Half-round edging (6), Power strip 1 (7) and the roll bubble level sit on the nook (5).", 13, -24.5, 1.3);
    ...
    side_list(48, 55, [
        ["1", "Bungee/shock-cord net", "x3 kitchen face + x2 bed face (bottom/middle tiers)", "unhook one corner to restock"],
        ["2", "Full-depth shelves", str("x", n, ", ", W, "\" x ", L, "\", 3/4\" ply"), "the carcass webs — glued + screwed into both side panels"],
        ...
        ["4", "Nook divider", str(W, "\" x ", headboard_nook_divider_h, "\", 1/2\" ply — TOP tier only, ", Yp, "\" from the bed face"), "screwed down into the top shelf + into both side panels"],
        ["5", "Personal nook", str(Yp, "\" deep strip of the top shelf, at ", headboard_personal_shelf_z, "\""), "phones, glasses, flashlights, USB charging"],
